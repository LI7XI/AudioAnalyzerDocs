function Initialize()
  ...
  MeasureBandsFP  = SKIN:MakePathAbsolute(SKIN:ReplaceVariables("#@#Measures\\AudioLevelBands.inc"))
  MeterSpectrumFP = SKIN:MakePathAbsolute(SKIN:ReplaceVariables("#@#Meters\\Spectrum\\AudioLevel.inc"))
...
function GenerateMeasureBands(Bands)
  ...
  for i = 0, (Bands - 1) do
    ...
    MeasureBands:write("Plugin=AudioLevel\n")
    MeasureBands:write("Parent=MeasureAudio\n")
    MeasureBands:write("Type=Band\n")
    MeasureBands:write("BandIdx=" .. i .. "\n")
    MeasureBands:write("\n")
  end
...
function GenerateMeterSpectrum(Bands)
  ...
  if Wave == 1 then 
    ...
    MeterSpectrum:write("; ---------------------------------------------------------------------------------------------\n")
    MeterSpectrum:write("Shape=Rectangle #BarX#,#MaxHeight#,#BarWidth#,(Neg([MeasureBand0]/1 * (#MaxHeight#-#MinHeight#) + #MinHeight#)),#Radius# | #Fill# | #Stroke#\n")
    MeterSpectrum:write("; ---------------------------------------------------------------------------------------------\n")
    
    for i = 1, (Bands - 1) do
      MeterSpectrum:write("Shape"..(i + 1).."=Rectangle #BarX#,#MaxHeight#,#BarWidth#,(Neg([MeasureBand".. i .."]/1 * (#MaxHeight#-#MinHeight#) + #MinHeight#)),#Radius# | #Fill# | #Stroke# | Offset (#BarXR#*" .. i .. "),(#BarYR#)\n")
      MeterSpectrum:write("; ---------------------------------------------------------------------------------------------\n")
    end
